...
class qtype_lcspeech extends question_type {
    ...
    public function extra_question_fields() {
        return array(
            'qtype_lcspeech_options', 'mediatype', 'speechphrase', 'timelimitinseconds', 'accent'
            , 'speechtype', 'hascontext', 'contextquestion', 'contextdescription', 'contextvalidanswerdescription'
            , 'scoringoption'
        );
    }
    ...
    public function save_other_table_details($db, $question) {
        ...
        $questionidcolname = $this->questionid_column_name();
        for ($i = 0; $i < 4; $i++) {
            $options = $db->get_record('qtype_lcspeech_feedback', array($questionidcolname => $question->id
                , 'from_range' => $question->from_range[$i], 'to_range' => $question->to_range[$i]));
            if (!$options) {
                $function = 'insert_record';
                $options = new stdClass();
                $options->$questionidcolname = $question->id;
        ...
        if (isset($question->correction_audio) && !empty($question->correction_audio)) {
            ...
            foreach ($question->correction_audio as $audio) {
                file_save_draft_area_files($audio, $question->context->id, $componentname, 'correction_audio'
                    , (int)$audio, array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 1));
                $fs = get_file_storage();
                $files = $fs->get_area_files($question->context->id, 'qtype_lcspeech', 'correction_audio'
                    , (int)$audio, '', false);
                if (empty($files)) {
                    continue;
                }
